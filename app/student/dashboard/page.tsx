      ...
      <main className="p-4 md:p-8">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500">{data.user.className} • {data.user.subjects.join(', ')}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
              ...
              <div className="space-y-4">
                {data.recentMaterials.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        <div className="p-3 bg-white rounded-xl shadow-sm">
                          {material.type === 'video' ? <IoPlay className="text-blue-500" /> : <IoDocumentText className="text-teal-500" />}
                        </div>
                        <div>
                          <h3 className="font-semibold text-gray-900">{material.title}</h3>
                          <p className="text-xs text-gray-500">{material.subject} • {new Date(material.createdAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                  ...
                  <p className="text-center py-8 text-gray-500">No materials available yet.</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-blue-600 to-teal-500 p-6 rounded-3xl text-white shadow-xl">
              ...
              <p className="text-blue-50/80 text-sm mb-6">Master your subjects with daily exams and instant feedback.</p>
                  ...
                  data.upcomingExams.map((exam, i) => (
                    <div key={i} className="flex items-center space-x-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-gray-500">{exam.subject} • Today</p>
                      </div>
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-gray-500">No exams scheduled for today.</p>
                )}
              </div>
            </div>
          </div>
